from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, QComboBox, QMessageBox, QHBoxLayout
)
...
class MaterialForm(QDialog):
    def __init__(self, material=None, parent=None):
        ...
        btn_back.clicked.connect(self.reject)  # Просто закрыть форму
    ...
    def save_material(self):
        ...
        try:
            ...
            if self.material:
                cursor.execute("""
                    UPDATE materials
                    SET name=%s, unit=%s, stock_quantity=%s, type_id=%s
                    WHERE material_id=%s
                """, (name, unit, stock, type_id, self.material[0]))
            else:
                cursor.execute("""
                    INSERT INTO materials (name, unit, stock_quantity, type_id)
                    VALUES (%s, %s, %s, %s)
                """, (name, unit, stock, type_id))
